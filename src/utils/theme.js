...
export async function extractThemeFromLogo(logoUrl) {
  try {
    ...
    const ink = luminance(primary) < 60 ? toHex(primary) : '#201d1a'
    // Noticeably tinted background in the logo's dominant hue — cards
    // stay white on top for readability, but the page itself should
    // clearly feel like "this restaurant's color", not a faint hint of it.
    const paperTint = toHex(mix(primary, [255, 255, 255], 0.8))
