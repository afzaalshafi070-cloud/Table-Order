...
export default function CustomerPortal() {
  ...
  const sendOrder = async (contact = {}) => {
    // Deal cart keys are "deal:<uuid>" — strip prefix so DB never sees invalid uuid text
    const items = Object.values(cart).map(({ id, name, price, qty, isDeal }) => {
      const raw = String(id)
      const isDealItem = isDeal || raw.startsWith('deal:')
      const cleanId = raw.startsWith('deal:') ? raw.slice(5) : raw
      return {
        id: cleanId,
        name,
        price,
        qty,
        ...(isDealItem ? { type: 'deal' } : {}),
      }
    })
