...
export default function CustomerPortal() {
    ...
    async function resolveSession() {
      ...
      if (error || !data) {
        ...
        try {
          ...
          // Backup lookup by table_id (if localStorage empty/cleared) — only safe
          // for real physical tables. Takeaway shares one generic table_id across
          // EVERY customer, so widening by table_id here would leak other
          // customers' pending takeaway orders into a fresh scan.
  ...
  const sendOrder = async (contact = {}) => {
    // Deals live in `deals` table, not `menu_items`.
    // DB trigger validates menu item UUIDs — so deals must NOT send an id.
    const items = Object.values(cart).map(({ id, name, price, qty, isDeal }) => {
      const raw = String(id)
      const isDealItem = Boolean(isDeal) || raw.startsWith('deal:')
      if (isDealItem) {
        return { name, price, qty, type: 'deal' }
      }
      return { id: raw, name, price, qty }
    })
