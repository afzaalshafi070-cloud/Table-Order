...
export default function CustomerPortal() {
  ...
  const sendOrder = async () => {
    ...
    // Simple client-side rate limit: 1 order every 20 seconds per table
    const now = Date.now()
    if (now - lastOrderAtRef.current < 20000) {
      setOrderError('Please wait a few seconds before placing another order.')
      return
    }
    ...
    if (error) {
      ...
      setOrderError('Could not place order. Please try again or call the waiter.')
      return
    }

    if (data) {
      lastOrderAtRef.current = Date.now()
  ...
  return (
    <div className="screen">
      ...
      <header className="header-in" style={{
          ...
          {orderError && (
            <div style={{
              ...
              <button
                ...
                style={{ marginLeft: 10, background: 'transparent', border: 'none', color: '#fecaca', fontWeight: 700 }}
              >✕</button>
            </div>
          )}
          {cartOpen && (
