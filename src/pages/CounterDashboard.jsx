...
export default function CounterDashboard() {
  ...
  const login = async (
    ...
    if (activeSession) {
      ...
      if (!stale) {
        setLoginError(
          'Ye restaurant already kisi doosri device par active hai. Pehle us device se Shift Close karein. (Ya 45 second wait karke dubara try karo)'
        )
        setLoginLoading(false)
        return
      }

      // Force-close the stale session (and any other active ones for safety)
      ...
      if (closeError) {
        ...
        setLoginError(
          'Purani session band nahi ho saki. Thodi der baad try karo. (' + (closeError.message || '') + ')'
        )
        setLoginLoading(false)
        return
      }
    ...
    // Extra safety: force-close ANY remaining active session for this restaurant+pin
    // (handles race conditions / multiple stuck rows)
    if (!silent) {
      await supabase
        .from('sessions')
        .update({
