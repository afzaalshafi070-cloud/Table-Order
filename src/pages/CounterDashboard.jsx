...
export default function CounterDashboard() {
  ...
  //    - ALWAYS resume the same session (orders stay intact).
  //    - Shift only ends when staff clicks "Close Shift".
  //    - Browser close / reopen / multiple logins do NOT create a new shift.
  //
  ...
  const login = async (
    ...
    // ------------------------------------------------------------
    // ACTIVE SHIFT ALREADY EXISTS → ALWAYS RESUME
    // (orders + history stay until staff clicks Close Shift)
    // ------------------------------------------------------------
    if (activeSession) {
      // Refresh heartbeat so we know this tab is alive
      await supabase
        .from('sessions')
    ...
    if (oldError) {
      setLoginError(
        'Purani shift check nahi ho saki. Dobara try karein. (' + (oldError.message || '') + ')'
      )
      setLoginLoading(false)
      return
    }
    ...
    if (oldSession) {
      ...
      if (newShiftError) {
        console.error('New shift insert failed:', newShiftError)

        // PostgreSQL unique violation.
        if (newShiftError.code === '23505') {
          setLoginError(
            'Ye restaurant abhi kisi doosri device par active ho gaya hai. 45 second wait karke dubara try karo. (Agar bar bar aaye to Supabase mein active sessions check karo)'
          )
        } else {
          setLoginError(
            'Naya shift create nahi ho saka: ' + (newShiftError.message || 'Unknown error')
          )
        }
      ...
      try {
        ...
        // Copy deals/combos to new shift
        const { data: oldDeals } = await supabase
          .from('deals')
          .select('name, description, price, original_price, photo_url, is_active, valid_from, valid_until, mood_tags, sort_order')
          .eq('session_id', oldSession.id)
          .order('sort_order', { ascending: true })

        if (oldDeals && oldDeals.length > 0) {
          const dealsCopy = oldDeals.map(d => ({
            session_id: newShift.id,
            name: d.name,
          ...
          await supabase.from('deals').insert(dealsCopy)
        }
      } catch (copyErr) {
        console.error('Failed to copy menu/areas/deals to new shift:', copyErr)
    ...
    if (createError) {
      console.error('First session create failed:', createError)

      if (createError.code === '23505') {
        setLoginError(
          'Ye restaurant abhi kisi doosri device par active ho gaya hai. 45 second wait karke dubara try karo.'
        )
      } else {
        setLoginError(
          'Session create nahi ho saka: ' + (createError.message || 'Supabase connection check karein.')
        )
      }
