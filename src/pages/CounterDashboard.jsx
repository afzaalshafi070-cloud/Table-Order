...
export default function CounterDashboard() {
  ...
  const login = async (
    ...
    if (activeSession) {
      ...
      const isLive = Number.isFinite(lastSeenMs) &&
        (Date.now() - lastSeenMs) <= SESSION_LEASE_MS

      // Another tab/device still sending heartbeat → block normal login
      if (isLive && !silent) {
        setLoginError(
          'Ye restaurant already kisi doosri device/tab par active hai. Pehle us se Shift Close karein, ya 45 second wait karke dobara try karo.'
        )
        setLoginLoading(false)
        return
      }

      // Same tab refresh (silent) OR stale session → resume SAME shift
      await supabase
        .from('sessions')
