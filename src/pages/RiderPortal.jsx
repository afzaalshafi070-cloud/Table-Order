...
function playAlarmLoop(stopRef) {
  try {
    const ctx = new (window.AudioContext || window.webkitAudioContext)()
    stopRef.current = () => { try { ctx.close() } catch {} }
    const beep = () => {
      if (stopRef.current._stopped) return
...
export default function RiderPortal() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'orders', filter: `session_id=eq.${sessionId}` }, (payload) => {
        const o = payload.new
        if (!o || o.fulfillment !== 'delivery' || o.area_name !== areaName) return
        if (seenRef.current.has(o.id)) return
        seenRef.current.add(o.id)
        setRideList(prev => [o, ...prev.filter(x => x.id !== o.id)])
        setAlertOrder(o)
        playAlarmLoop(alarmStopRef)
        try {
          if (Notification.permission === 'granted') {
            new Notification(`${restaurant?.restaurant_name || 'Order'} — NEW DELIVERY`, {
              body: `${areaName}: ${o.customer_address || o.customer_name || ''}`,
  ...
  // Share GPS while page open → all active rides in this area get lat/lng
  useEffect(() => {
  ...
  return (
    <div className="screen" style={{ background: 'var(--brand-paper-tint, var(--paper))' }}>
      ...
      <div style={{ padding: 16, fontSize: 13, color: '#7a7264' }}>
        Page <b>open</b> rakhein (Home Screen pe add behtar). Naya order → sound + vibrate.
        Screen-off pe browser limit hai — page background mein bhi notification try hogi.
      </div>
      ...
      <div style={{ padding: '0 16px 80px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {rideList.map(o => (
          <div key={o.id} style={{
            ...
            <div style={{ fontWeight: 700, color: 'var(--clay)' }}>DELIVERY · {o.status}</div>
