  ...
  useEffect(() => {
    ...
    const push = (lat, lng) => {
      ...
      let u = supabase
        ...
        .eq('fulfillment', 'delivery')
        .in('status', ['pending', 'cooking'])
      if (!isAllAreas) u = u.eq('area_name', areaName)
      u.then(() => {})
    }
  ...
  return (
    <div className="screen" style={{ background: 'var(--brand-paper-tint, var(--paper))' }}>
      ...
      <div style={{ padding: 16, fontSize: 13, color: '#7a7264' }}>
        Page <b>open</b> rakhein (screen off na hone dein). Pehli dafa <b>ENABLE ALARM</b> zaroor press karein. Naya order → ring + vibrate + popup jab tak Dismiss na dabao.
