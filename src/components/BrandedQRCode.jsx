...
export default function BrandedQRCode({
  ...
  useEffect(() => {
    ...
    async function draw() {
      ...
      try {
        // Generate clean QR on a temporary canvas first (higher quiet zone + strong error correction)
        ...
        if (logoUrl) {
          try {
            ...
            // Keep logo small (≈17% of QR side) so error-correction level H can recover easily
            const logoBox = Math.round(qrSize * 0.17)
            const pad = 6
            ...
            ctx.fillStyle = '#ffffff'
            ctx.beginPath()
            ctx.roundRect(x - pad, y - pad, logoBox + pad * 2, logoBox + pad * 2, 10)
            ctx.fill()
            ...
            ctx.shadowBlur = 6
            ctx.beginPath()
            ctx.roundRect(x, y, logoBox, logoBox, 8)
            ctx.clip()
