    ...
    <div className="ticket-in" style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}>
        <strong style={{ fontFamily: 'var(--mono)', fontSize: 14 }}>
          {order.fulfillment === 'delivery' ? 'DELIVERY' : order.fulfillment === 'takeaway' ? 'TAKEAWAY' : locationLabel(order.table_id)}
        </strong>
        <span style={{ fontSize: 11, fontWeight: 700, color: meta.color }}>{meta.label.toUpperCase()}</span>
      </div>
      {order.fulfillment === 'delivery' && (
        <div style={{ fontSize: 12, background: '#e0f2fe', borderRadius: 8, padding: '6px 8px' }}>
          {order.area_name && <div><b>Area:</b> {order.area_name}</div>}
      ...
      {order.fulfillment === 'takeaway' && (order.customer_name || order.customer_phone) && (
        <div style={{ fontSize: 12, color: '#5a5346' }}>{order.customer_name} {order.customer_phone}</div>
      )}
