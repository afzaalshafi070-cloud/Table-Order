...
export function useRealtimeOrders(sessionId, options = {}) {
  ...
  useEffect(() => {
    ...
    const fetchInitial = async () => {
      const [{ data: orderRows, error: orderError }, { data: alertRows, error: alertError }] = await Promise.all([
        supabase.from('orders').select('*').eq('session_id', sessionId).order('created_at', { ascending: false }).limit(100),
    ...
    const subscribe = () => {
      ...
      channelsRef.current = [orderChannel, alertChannel]
    }
