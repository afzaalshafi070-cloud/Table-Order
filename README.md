...
- The restaurant then generates a QR code per table pointing at:
  ...
  ```
  https://yourapp.com/order/<restaurant-id>/<qr-secret>/<table-number>
  ```
...
the The staff PIN is used only for the counter login; customer/rider QR links use a per-shift qr_secret., similar to a Wi-Fi password. Before
